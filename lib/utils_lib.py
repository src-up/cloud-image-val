...
LOG = logging.getLogger('civ.utils_lib')
HTTP_PORT = 8000
...
def get_cfg(cfg_file = None, log=None):
    # Config file
    if not cfg_file:
        cfg_file = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "os_tests_cfg", "os-tests.yaml")
    if log is None:
        LOG_FORMAT = '%(levelname)s:%(message)s'
        log = logging.getLogger(__name__)
        logging.basicConfig(level=logging.INFO, format=LOG_FORMAT)
...
def init_case(test_instance):
    ...
    test_instance.log.info("Code Version: v{}".format('0.4.0'))
    test_instance.log.info("Code Date: {}".format('20260114'))
...
def find_word(test_instance, check_str, log_keyword=None, baseline_dict=None, skip_words=None, case=None):
    ...
    if not baseline_dict:
        # Baseline data file
        baseline_file = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "os_tests", "baseline_log.json")
