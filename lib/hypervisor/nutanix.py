...
from lib.vm_lifecycle.base import VMResource,StorageResource,UnSupportedAction,UnSupportedStatus
...
class PrismSession(object):
    ...
    def post(self, url, data):
        ...
        try:
            self.r = self.s.post(url, json=data, headers=self.headers, verify="/etc/pki/ca-trust/source/anchors/nutanix_114_crt.crt")
    ...
    def put(self, url, data):
        ...
        try:
            self.r = self.s.put(url, json=data, headers=self.headers, verify="/etc/pki/ca-trust/source/anchors/nutanix_114_crt.crt")
        except requests.exceptions.RequestException:
            logging.error("Failed to connect to Prism")
        # convert response to json
        return self.__json()
    ...
    def delete(self, url):
        ...
        try:
            self.r = self.s.delete(url, headers=self.headers, verify="/etc/pki/ca-trust/source/anchors/nutanix_114_crt.crt")
        except requests.exceptions.RequestException:
            logging.error("Failed to connect to Prism")
        # convert response to json
        return self.__json()
