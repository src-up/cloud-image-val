from .base import VMResource, StorageResource, NetworkResource, UnSupportedAction, UnSupportedStatus
...
class OCIVM(VMResource):
    ...
    def __init__(self, params, vendor="redhat"):
        ...
        if params.get('region'):
            self.oci_config['region'] = params.get('region')
    ...
    def start(self, wait=True):
        ...
        if wait:
            ...
            if self._instance.lifecycle_state != 'RUNNING':
                ...
                return False
            LOG.info("Instance is in running state, waiting for SSH...")
            from lib import ssh_lib
            # Instance was STOPPED — SSH is already down. Use _wait_for_ssh_up (Phase 2 only).
            ssh_lib._wait_for_ssh_up(self.params.get('remote_node'), timeout_seconds=120)
            self.floating_ip
        return True
    ...
    def wait_ssh_ready(self, timeout_seconds=120):
        """Wait for SSH to be fully operational (used after in-VM reboots)."""
        from lib import ssh_lib
        ssh_lib.wait_for_ssh_ready_after_reboot(self.params.get('remote_node'), timeout_seconds=timeout_seconds)
    ...
    def reboot(self, wait=False):
        ...
        try:
            ...
            if wait:
                # Wait for the instance to leave RUNNING (reboot has started)
                LOG.info("Waiting for instance to start rebooting...")
                for _ in range(60):
                    state = self.compute_client.get_instance(self.id).data.lifecycle_state
                    if state != 'RUNNING':
                        break
                    time.sleep(3)
                # Now wait for it to return to RUNNING
                oci.wait_until(
                    self.compute_client,
                    self.compute_client.get_instance(self.id),
                    'lifecycle_state',
                ...
                LOG.info("Instance is back in RUNNING state after reboot, waiting for SSH...")
                from lib import ssh_lib
                # OCI state machine confirmed the instance cycled — SSH is already down.
                # Use _wait_for_ssh_up (Phase 2 only) to avoid a spurious 120s Phase 1 wait.
                ssh_lib._wait_for_ssh_up(self.params.get('remote_node'), timeout_seconds=120)
            elif 'BM.' in self.shape:
                time.sleep(120)
            else:
                time.sleep(5)
            return True
